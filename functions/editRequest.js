...
const editRequest = (request , db, address, area, requestType, requestDescription, props) => {
  const docs = request.request_ID;
        ...
        try{
          console.log(docs),
            ...
            db.collection('Assistance Request').where('request_ID', '==', docs).get()
            .then(snapshot => {
              ...
              db.collection("users").doc(data.vid).get().then(snapshot => {
                ...
                body: JSON.stringify({
                    ...
                    title: 'Changes were made to a request type: ' + data.request_Type  ,
